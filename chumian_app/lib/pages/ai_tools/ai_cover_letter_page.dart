...
/// ============================================================
/// AICoverLetterPage —— AI 求职信
/// 目标公司，职位，个人亮点，生成求职信，定制化建议，复制
/// ============================================================
class AICoverLetterPage extends StatefulWidget {
  const AICoverLetterPage({super.key});

  @override
  State<AICoverLetterPage> createState() => _AICoverLetterPageState();
}

class _AICoverLetterPageState extends State<AICoverLetterPage>
  ...
  static const List<String> _sampleCompanies = ['字节跳动', '腾讯', '阿里巴巴', '美团', '小米'];
  static const List<String> _samplePositions = ['前端开发工程师', '产品经理', 'UI设计师', '运营专员'];
  static const List<String> _sampleHighlights = ['3年大厂经验，主导过千万级用户产品', '精通React/Vue，有完整项目从0到1经验', '擅长数据分析，曾通过优化提升转化率30%'];
  ...
  Future<void> _generateLetter() async {
    if (_companyController.text.trim().isEmpty || _positionController.text.trim().isEmpty) {
      MiuixToast.show(context, message: '请填写公司和职位', type: MiuixToastType.warning);
      return;
    }
    setState(() {
      _isGenerating = true;
    ...
    try {
      final result = await ApiService.aiToolComplete(
        systemPrompt: '你是一位专业的求职信撰写专家。请根据用户提供的职位信息和个人背景，撰写一封专业、有说服力的求职信。',
        userInput: "职位："+_positionController.text+"\n个人背景："+_backgroundController.text,
      );
      if (mounted) {
  ...
  Future<void> _copyResult() async {
    ...
    if (mounted) MiuixToast.show(context, message: '已复制求职信', type: MiuixToastType.success);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: MiuixAppBar(title: 'AI 求职信', backgroundColor: MiuixColors.background),
      body: SingleChildScrollView(
        padding: const EdgeInsets.fromLTRB(16, 8, 16, 32),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildAnimatedItem(_buildCompanyPositionInput(), 0),
            const SizedBox(height: 16),
            _buildAnimatedItem(_buildHighlightsInput(), 1),
  ...
  Widget _buildCompanyPositionInput() {
    return MiuixCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('目标信息', style: TextStyle(fontSize: MiuixFontSize.lg, fontWeight: FontWeight.w600, color: MiuixColors.textPrimary)),
          const SizedBox(height: 12),
          MiuixInput(controller: _companyController, hintText: '目标公司名称', prefixIcon: Icons.business),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: List.generate(_sampleCompanies.length, (index) {
              return MiuixChip(label: _sampleCompanies[index], onTap: () => _companyController.text = _sampleCompanies[index], style: MiuixChipStyle.normal);
            }),
          ),
          const SizedBox(height: 12),
          MiuixInput(controller: _positionController, hintText: '目标职位', prefixIcon: Icons.work),
          const SizedBox(height: 12),
          Wrap(
            spacing: 8,
            runSpacing: 8,
  ...
  Widget _buildHighlightsInput() {
    return MiuixCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('个人亮点', style: TextStyle(fontSize: MiuixFontSize.lg, fontWeight: FontWeight.w600, color: MiuixColors.textPrimary)),
          const SizedBox(height: 12),
          MiuixInput(
            controller: _highlightsController,
            hintText: '简述你的核心优势，如：3年经验、主导过XX项目、擅长XX...（选填）',
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: List.generate(_sampleHighlights.length, (index) {
              return MiuixChip(
                label: _sampleHighlights[index].length > 16 ? '${_sampleHighlights[index].substring(0, 16)}...' : _sampleHighlights[index],
                onTap: () => _highlightsController.text = _sampleHighlights[index],
  ...
  Widget _buildLoadingCard() {
    return MiuixCard(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        const Text('AI 正在撰写求职信...', style: TextStyle(fontSize: MiuixFontSize.md, color: MiuixColors.textSecondary)),
        const SizedBox(height: 8),
        Text('${_companyController.text} · ${_positionController.text}', style: const TextStyle(fontSize: MiuixFontSize.sm, color: MiuixColors.textTertiary)),
      ]),
    );
  }
    ...
    return MiuixCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(gradient: const LinearGradient(colors: MiuixColors.primaryGradient), borderRadius: MiuixRadius.smRadius), child: const Icon(Icons.mark_email_read, color: Colors.white, size: 20)),
              const SizedBox(width: 10),
              const Expanded(child: Text('求职信', style: TextStyle(fontSize: MiuixFontSize.lg, fontWeight: FontWeight.w600, color: MiuixColors.textPrimary))),
              MiuixIconButton(icon: Icons.copy, style: MiuixIconButtonStyle.outlined, size: 36, iconSize: 18, onPressed: _copyResult),
              const SizedBox(width: 8),
              MiuixIconButton(icon: Icons.refresh, style: MiuixIconButtonStyle.outlined, size: 36, iconSize: 18, onPressed: _generateLetter),
            ],
          ),
          const SizedBox(height: 16),
          Container(
          ...
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: MiuixButton(label: '复制求职信', icon: Icons.copy_all, type: MiuixButtonType.primary, onPressed: _copyResult)),
        ],
      ),
    );
