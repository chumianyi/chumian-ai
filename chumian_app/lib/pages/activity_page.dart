...
class _ActivityPageState extends State<ActivityPage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildGuessGame() {
    return MiuixGlassCard(borderRadius: MiuixRadius.lg, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: MiuixColors.success.withValues(alpha: 0.1), borderRadius: MiuixRadius.xsRadius), child: Text('进行中', style: TextStyle(color: MiuixColors.success, fontSize: MiuixFontSize.xs, fontWeight: FontWeight.w500))),
      ]),
      const SizedBox(height: 20),
  ...
  Widget _buildActivityCard(ActivityItem activity) {
    return MiuixRipple(borderRadius: MiuixRadius.lg, child: GestureDetector(onTap: () {
      ...
      Container(width: 56, height: 56, decoration: BoxDecoration(gradient: LinearGradient(colors: activity.color), borderRadius: MiuixRadius.mdRadius, boxShadow: [BoxShadow(color: activity.color.last.withValues(alpha: 0.3), blurRadius: 8, offset: const Offset(0, 4))]), child: Icon(activity.icon, color: Colors.white, size: 28)),
      const SizedBox(width: 14),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Expanded(child: Text(activity.title, style: TextStyle(color: MiuixColors.textPrimary, fontSize: MiuixFontSize.lg, fontWeight: FontWeight.w600))), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: (activity.status == '进行中' ? MiuixColors.success : activity.status == '即将开始' ? MiuixColors.warning : MiuixColors.primary).withValues(alpha: 0.1), borderRadius: MiuixRadius.xsRadius), child: Text(activity.status, style: TextStyle(color: activity.status == '进行中' ? MiuixColors.success : activity.status == '即将开始' ? MiuixColors.warning : MiuixColors.primary, fontSize: MiuixFontSize.xs, fontWeight: FontWeight.w500)))],),
        const SizedBox(height: 4),
